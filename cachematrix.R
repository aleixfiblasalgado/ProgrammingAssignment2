...
## to cache such that next time the user calculates the matrix inverse,
## the previously saved value is returned instead of repeating the calculations
...
makeCacheMatrix <- function(x = matrix()) {
  ## create a matrix object x and some associated sub-functions
  ## define the cache m
  m <- NULL
  set <- function(y) {
  ...
  get <- function() x ## return the matix x
...
## Otherwise, it calculates the matrix inverse and sets the calue of the inverse in the cache via "setinverse" funcion.
